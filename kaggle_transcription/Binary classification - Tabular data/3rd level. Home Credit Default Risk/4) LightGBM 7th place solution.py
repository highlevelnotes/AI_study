...
def add_ratios_features(df):
    df['BUREAU_INDOME_CREDIT_RATIO'] = df['BUREAU_AMT_CREDIT_SUM_MEAN'] / df['AMT_INCOME_TOTAL']
...
def kfold_lightgbm_sklearn(data, categorical_feature=None):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(df[predictors], df['TARGET'])):
        ...
        if not categorical_feature:
            clf.fit(train_x, train_y, eval_set=[(train_x, train_y), (valid_x, valid_y)], eval_metric='auc', callbacks=[log_evaluation(400), early_stopping(EARLY_STOPPING)])
        else:
            clf.fit(train_x, train_y, eval_set=[(train_x, train_y), (valid_x, valid_y)], eval_metric='auc', feature_name=list(df[predictors].columns), categorical_feature=categorical_feature, callbacks=[log_evaluation(400), early_stopping(EARLY_STOPPING)])
        
        oof_preds[valid_idx] = clf.predict_proba(valid_x, num_iteration=clf.best_iteration_)[:, 1]
        sub_preds += clf.predict_proba(test[predictors], num_iteration=clf.best_iteration_)[:,1] / folds.get_n_splits()
...
def drop_application_columns(df):
    '''Drop features based on permutation feature importances. '''
    drop_list = [
        ...
        'FLOORSMIN_AVG', 'LANDAREA_AVG', 'COMMONAREA_AVG', 'LANDAREA_MEDI',
...
def get_pos_cash(path, num_rows=None):
    ...
    drop_features = [
        'POS_NAME_CONTRACT_STATUS_Canceled_MEAN', 'POS_NAME_CONTRACT_STATUS_Amortized debt_MEAN', 'POS_NAME_CONTRACT_STATUS_XNA_MEAN'
    ]
...
def reduce_memory(df):
    ...
    print('Initial df memory usage is {:.2f} MB for {} columns'.format(start_mem, len(df.columns)))
