...
def produce_donations(period_rng, user_behavior, num_emails, use_id, user_join_year):
    ...
    user_gives_idx = np.where(user_gives >= donation_amounts)[0]
    if len(user_gives_idx) > 0:
        user_gives_idx = user_gives_idx[-1]
    else:
        user_gives_idx = 0
    ...
    if num_times_gave <= 0 or len(period_rng) == 0:
        return pd.DataFrame({'user': [], 'amount': [], 'timestamp': []})
    
    times = np.random.randint(0, len(period_rng), num_times_gave)

    donations = pd.DataFrame({'user': [], 'amount': [], 'timestamp': []})
    for n in range(num_times_gave):
        new_donation = pd.DataFrame({'user': [use_id],
...
for idx in range(yearJoined.shape[0]):
    ...
    if len(info) == len(user_rng):
        new_emails = pd.DataFrame({'user': [idx] * len(info),
                                 'week': [str(r.start_time) for r in user_rng],
                                 'emailsOpened': info})
        emails = pd.concat([emails, new_emails], ignore_index=True)

    new_donations = produce_donations(user_rng, user_behaviors[idx],
                                    sum(info), idx, join_date.year)
    donations = pd.concat([donations, new_donations], ignore_index=True)
...
print(f"데이터셋 생성 완료:")
print(f"yearJoined: {yearJoined.shape}")
